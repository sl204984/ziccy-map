...
function setColor(color) {
	if (color) {
		console.log(color);
		_map.view.basicLayer.customStyle(featureTypeDict[featureType], {value: color});
		_map.view.refreshMap();
	}
	$("#colorpicker").hide();
}

function clearColorStyle(){
	_map.view.basicLayer.clearCustomStyle();
	_map.view.refreshMap();
}
function toggleFeatureVisibilty(img, t) {
	...
	_map.view.basicLayer.toggleFilter(innerTypes);
	_map.view.refreshMap();
	img.src = 'img/out/eye'+(_map.view.basicLayer.checkFilter(inType)? "1": "")+'.png';
}
function toggleFeatureBold(img, t) {
	var innerTypes = featureTypeDict[t], inType = innerTypes instanceof Array? innerTypes[0]: innerTypes; 

	_map.view.basicLayer.boldFeature(innerTypes);
	_map.view.refreshMap();
	img.src = 'img/out/sun'+(_map.view.basicLayer.checkBoldFeature(inType) == null? "": "1")+'.png';
}
